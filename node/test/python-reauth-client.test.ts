...
test("Python reauth client returns worker SSO without exposing account credentials", async () => {
  const client = new PythonReauthClient({
    ...
    fetchImpl: async (_input, init) => {
      ...
      return new Response(JSON.stringify({ ok: true, sso: "private-sso" }), { status: 200 });
    },
  });
  assert.equal(await client.reauthenticate("member@example.test", "private-password"), "private-sso");
});
